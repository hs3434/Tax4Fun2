makeFunctionalPrediction <- function(
  ...
  if (include_user_data) {
    ...
    # 对于每个在用户数据库匹配到的物种，如果使用标准数据库也匹配到了，则使用用户数据库的匹配id代替标准数据库的id；
    # 如果使用标准数据库没有匹配到，则把用户数据库的匹配结果追加到标准结果中。
    for (i in seq_len(nrow(user_blast_result_reduced)))
    {
  ...
  # unknown_fraction1 是匹配结果相对结果相对原始otu_table，表达量大于0的otu的减少比率。体现了有多少otu没有匹配上。
  # unknown_fraction2 是合并结果相对结果相对原始otu_table，表达量大于0的otu的减少比率。体现了有多少重复匹配otu。
  ...
  # 这一步会产生inf 和 na， 但是不用担心， ko2ptw$nrow 会把这些值过滤掉。
  ...
  if (write_counts) {
    pathway_prediction_counts <- pathway_prediction_counts / ko_list$ptw_count
    pathway_prediction_counts <- aggregate(x = pathway_prediction_counts[ko2ptw$nrow, ], by = list(ko2ptw$ptw), sum)
  }
  if (ncol(pathway_prediction) >= 3) {
    col_sums <- colSums(pathway_prediction[, -1])
    col_sums[col_sums == 0] <- 1  # 防止除0
